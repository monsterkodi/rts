...
Path = (function ()
{
    ...
    Path.prototype["toString"] = function ()
    {
        ...
        s = this.name
        s += '\n'
        for (var _37_17_ = i = 0, _37_21_ = this.tracks.length; (_37_17_ <= _37_21_ ? i < this.tracks.length : i > this.tracks.length); (_37_17_ <= _37_21_ ? ++i : --i))
        {
            s += this.tracks[i].name
            s += ' '
            if (this.revers[i])
            {
                s += this.tracks[i].node[1].name + ' ◂ ' + this.tracks[i].node[0].name
            }
            else
            {
                s += this.tracks[i].node[0].name + ' ▸ ' + this.tracks[i].node[1].name
            }
            s += '\n'
        }
        return s
    ...
    Path.prototype["onDelTrack"] = function (track)
    {
        ...
        {
            _k_.assert(".", 59, 12, "assert failed!" + " track !== this.train.track", track !== this.train.track)
            _k_.assert(".", 60, 12, "assert failed!" + " this.currentTrack() === this.train.track", this.currentTrack() === this.train.track)
            if (this.currentTrack() !== this.train.track)
            {
                console.log(this.currentTrack().name,((_62_48_=this.train) != null ? (_62_55_=_62_48_.track) != null ? _62_55_.name : undefined : undefined))
                return
            }
    ...
    {
        _k_.assert(".", 82, 8, "assert failed!" + " this.tracks.length > 1", this.tracks.length > 1)
    ...
    Path.prototype["getLength"] = function ()
    {
        ...
        cd = 0
        for (var _91_18_ = ti = 0, _91_22_ = this.tracks.length; (_91_18_ <= _91_22_ ? ti < this.tracks.length : ti > this.tracks.length); (_91_18_ <= _91_22_ ? ++ti : --ti))
        {
    ...
    Path.prototype["deltaAtIndex"] = function (index)
    {
        ...
        cd = 0
        for (var _102_18_ = ti = 0, _102_22_ = index; (_102_18_ <= _102_22_ ? ti < index : ti > index); (_102_18_ <= _102_22_ ? ++ti : --ti))
        {
            if (this.tracks[ti].curve)
            {
                cd += this.tracks[ti].curve.getLength()
    ...
    {
        return this.nodes[this.normIndex(this.indexAtDelta(d) - 1)]
    }

    Path.prototype["prevTrack"] = function ()
    {
        return this.tracks[this.normIndex(this.currentIndex() - 1)]
    }
    ...
    {
        return this.tracks[this.normIndex(this.indexAtDelta(d))]
    }
    ...
    Path.prototype["normIndex"] = function (i)
    {
        var length
